...
public:
	...
	void init_order()		{
		...
		// Initialized the electron density.
		cout<<"Init Density .."<<endl;
		initElectronDensity	(" Bi:0  LS:0 Fe:2 _Fe:2 Mn:0.7 O:2");

		// Manually create a small Charge-Density-Wave.
		double den_cdw = VAR("CDW", 0.0).real();
		while (site_iterate()) {
			auto si = getSite();
			...
			if (si.Name() == "Mn") {
				...
				if (mag_sign > 0)	initDenOrder(si, si.Name()+" den") += den_cdw;
				else				initDenOrder(si, si.Name()+" den") -= den_cdw;
			}
		}
		...
		double has_mag =  VAR("has_mag").real();
		...
		while (site_iterate()) {
			...
			if ( has_mag != 0)
				if (si.Name() == "_Fe" or si.Name() == "Fe" or si.Name() == "Mn") {
					int rx=-1, ry=-1, rz=-1;
					if (int(si.pos[0])%2 ) rx=1;
					if (int(si.pos[1])%2 ) ry=1;
					if (int(si.pos[2])%2 ) rz=1;
