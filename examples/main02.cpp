...
public:
	...
	void init_order()		{
		...
		alpha_Mu = VAR("alpha_Mu").real();
		newDenMix= VAR("newDenMix").real();
		
		// Construct the full pack of lattice bond around an Atom.
		LatticeBond		lat_bnd(Lat, 10, 5.1);
		
		// Construct the long-range iteration
		while (site_iterate()) {
			auto si = getSite();
			auto bds = lat_bnd.atom_bondings(si);
			
			for (unsigned i=0; i<bds.size(); i++) {
				// Construct the On-site Coulomb order
				auto	sj		= bds[i].first;
				auto	bond	= bds[i].second;
				...
				if (	si.Name() != "Bi" and si.Name() != "LS" and si.Name() != "VA"
					...
					//cout<<bond<<" "<<si.Name()<<"-"<<si.AtomIndex()<<" "<<sj.Name()<<"-"<<sj.AtomIndex()<<endl;
				}
			}
		}
	}
	...
	void Hamiltonian()		{
		...
		for (unsigned i=0; i<coulomb_iteration.size(); i++) {
			...
			*Coulomb_Screening(2.5, distance);
